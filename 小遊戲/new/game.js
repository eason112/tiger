...
function startGame() {
  ...
  function generateHoles() {
    ...
    for (let i = 0; i < MaxnumberOfImages; i++) {
      ...
      img.src = '/小遊戲/new/images/hole.png';  // 初始的可點擊叉叉
      //console.log(img.src);
...
// 計算圖片的 Y 位置，以百分比表示
      ...
      img.addEventListener('click', handleClick);
        ...
        img.src = '/小遊戲/new/images/newhole.png';  // 新的圖片來源
        ...
        tool.src = '/小遊戲/new/images/tool.png';
        ...
        if(isRare)
        {
          ...
          ball.src = '/小遊戲/new/images/rareball.png';  // 設定球的圖片
          ball.alt = 'Ball';
        ...
        else
        {
          const ball = document.createElement('img');
          ball.classList.add('ball');  // 添加球的樣式
          ball.src = '/小遊戲/new/images/ball.png';  // 設定球的圖片
          ball.dataset.scored='false';
          ball.alt = 'Ball';
          ball.style.left = beachRect.left + randomX + 'px';  // 設置球的位置（與圖片相同）
          ball.style.top = beachRect.top + randomY + 'px';
...
function launchBall(ball,startX, startY, targetX, targetY) {
  ...
  console.log(targetX-ball.x,targetY-ball.y);
   // 動態創建唯一的動畫名稱
...
function placeBucket(background,x, y) {
  ...
  bucket.src = '/小遊戲/new/images/box.png';  // 設置桶子的圖片路徑
